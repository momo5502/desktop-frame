...
namespace cef
{
	cef_ui_lockscreen_handler::~cef_ui_lockscreen_handler()
	{
		this->stop_watcher();
	}

	void cef_ui_lockscreen_handler::fill_window_info(CefWindowInfo& window_info) const
	{
	...
	void cef_ui_lockscreen_handler::OnDocumentAvailableInMainFrame(const CefRefPtr<CefBrowser> browser)
	{
...
(function () {
  ...
  document.head.appendChild(script);
	...
	void cef_ui_lockscreen_handler::OnAfterCreated(CefRefPtr<CefBrowser> browser)
	{
	...
	bool cef_ui_lockscreen_handler::DoClose(CefRefPtr<CefBrowser> browser)
	{
		this->stop_watcher();
	...
	void cef_ui_lockscreen_handler::stop_watcher()
	{
	...
	void cef_ui_lockscreen_handler::start_watcher(HWND window)
	{
		this->stop_watcher();
		...
		this->size_watcher_ = std::thread([window, this]
		{
			...
			{
				const auto logon_window = FindWindowA("LogonUI Logon Window", nullptr);
				if (logon_window)
				{
					SetForegroundWindow(logon_window);

					RECT rect{};
					GetWindowRect(logon_window, &rect);
